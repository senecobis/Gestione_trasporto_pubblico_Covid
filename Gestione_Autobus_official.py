...
for j in range(0, K_AUTOBUS):
 ...
 # vincolo 2
 # per ogni tratta un solo autobus può essere assegnato o nessuno
...
# funzione obiettivo
modello.objective = mip.maximize(mip.xsum(mip.xsum(x[i][j]*Tratte[i][EXTRA_PASSENGERS] for i in range(1,n_di_tratte) )for j in range(K_AUTOBUS)))

totale_pass = 0
for i in range(1,n_di_tratte):
    totale_pass = totale_pass + Tratte[i][EXTRA_PASSENGERS]
